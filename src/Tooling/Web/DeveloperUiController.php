...
HTML;
   ...
   private function buildScaffoldPreview(string $tableName, array $schema, array $artifacts, string $routingType = 'attribute'): array
   {
      ...
      if (in_array('model', $artifacts, true)) {
         ...
         $modelConstructorParameters = $this->modelConstructorParameterLines($columns);
         $modelConstructorAssignments = $this->modelConstructorAssignmentLines($columns);
         $modelToArrayLines = $this->modelToArrayLines($columns);
 ...
 * Extend this class in `{$this->namespaceRoot}\\Models\\{$entity}`.
 */
class {$entity}Base
{
{$modelProperties}
...
/**
 * User-editable {$entityLower} model.
 *
 * Add domain-specific helpers/behavior here. Regeneration updates the
 * paired base class only (`{$this->namespaceRoot}\\Models\\Base\\{$entity}Base`).
 */
...
PHP
      ...
      if (in_array('repository', $artifacts, true)) {
         ...
         $sampleArgs = $this->modelConstructorSampleArguments($columns, 1);
...
/**
 * @generated by Celeris scaffolding. Do not edit directly.
 * Extend this class in `{$this->namespaceRoot}\\Repositories\\{$entity}Repository`.
 */
...
{$sampleArgs}
      ...
      \$this->rows[\$sample->{$keyProperty}] = \$sample;
   }
   ...
   public function find({$keyType} \$id): ?{$entity}
   {
   ...
   {
      \$this->rows[\$model->{$keyProperty}] = \$model;
      return \$model;
   }

   public function delete({$keyType} \$id): bool
   {
...
/**
 * @generated by Celeris scaffolding. Do not edit directly.
 * Extend this class in `{$this->namespaceRoot}\\Services\\{$entity}Service`.
 */
class {$entity}ServiceBase
{
   ...
   {
      return array_map(
         static fn({$entity} \$model): array => \$model->toArray(),
         \$this->repository->all(),
      );
   }

   public function getOrFail({$keyType} \$id): {$entity}
   {
   ...
   public function show({$keyType} \$id): {$entity}
   {
...
{$updateArgs}
   ...
   public function remove({$keyType} \$id): void
   {
...
PHP;
    ...
    * @param array<int, array<string, mixed>> $columns
    */
   private function modelPropertyLines(array $columns): string
   {
      $lines = [];
      foreach ($columns as $column) {
         $name = (string) ($column['name'] ?? '');
         ...
         $lines[] = sprintf('   public %s $%s;', $this->columnPhpType($column), $this->camel($name));
      }
    ...
    * @param array<int, array<string, mixed>> $columns
    */
   private function modelConstructorParameterLines(array $columns): string
   {
      $lines = [];
      foreach ($columns as $column) {
         $name = (string) ($column['name'] ?? '');
         ...
         $lines[] = sprintf('      %s $%s,', $this->columnPhpType($column), $this->camel($name));
      }
    ...
    * @param array<int, array<string, mixed>> $columns
    */
   private function modelConstructorAssignmentLines(array $columns): string
   {
      $lines = [];
      foreach ($columns as $column) {
         $name = (string) ($column['name'] ?? '');
         ...
         $property = $this->camel($name);
         $lines[] = sprintf('      $this->%s = $%s;', $property, $property);
      }
    ...
    * @param array<int, array<string, mixed>> $columns
    */
   private function modelToArrayLines(array $columns): string
   {
      $lines = [];
      foreach ($columns as $column) {
         $name = (string) ($column['name'] ?? '');
         ...
         $lines[] = sprintf("         '%s' => \$this->%s,", $this->escapeSingleQuoted($name), $this->camel($name));
      }
    ...
    * @param array<int, array<string, mixed>> $columns
    */
   private function modelConstructorSampleArguments(array $columns, int $index): string
   {
      $lines = [];
      foreach ($columns as $column) {
         $name = (string) ($column['name'] ?? '');
    ...
    * @param array<int, array<string, mixed>> $columns
    */
   private function modelConstructorPayloadArguments(array $columns, string $payloadVariable, string $fallbackVariable): string
   {
      $lines = [];
      foreach ($columns as $column) {
         $name = (string) ($column['name'] ?? '');
         ...
         $property = $this->camel($name);
         $default = $this->defaultLiteral($column['default'] ?? null);
         if ($fallbackVariable === 'null') {
            $fallback = $default ?? ((bool) ($column['nullable'] ?? false) ? 'null' : $this->sampleLiteralForType($name, strtolower((string) ($column['type'] ?? '')), 1));
         } else {
            $fallback = $fallbackVariable . '->' . $property;
         }

         $lines[] = sprintf("         %s['%s'] ?? %s,", $payloadVariable, $this->escapeSingleQuoted($name), $fallback);
      }
    ...
    * @param array<string, mixed> $column
    */
   private function columnPhpType(array $column): string
   {
      $rawType = strtolower((string) ($column['type'] ?? ''));
      $nullable = (bool) ($column['nullable'] ?? false);

      $type = 'string';
      if (str_contains($rawType, 'bigint') || str_contains($rawType, 'smallint') || str_contains($rawType, 'int')) {
         $type = 'int';
      } elseif (
         str_contains($rawType, 'decimal')
         || str_contains($rawType, 'double')
    ...
    * @return array<string, mixed>
    */
   private function repositoryKeyColumn(array $columns, array $primary): array
   {
      $preferred = $primary[0] ?? '';
      foreach ($columns as $column) {
         $name = (string) ($column['name'] ?? '');
         if ($name !== '' && ($name === $preferred || ($preferred === '' && (bool) ($column['primary'] ?? false)))) {
            return $column;
         }
      }
    ...
    * @param array<string, mixed> $column
    */
   private function repositoryKeyType(array $column): string
   {
      return $this->columnPhpType($column) === 'int' ? 'int' : 'string';
   }
